...
import {
  Plus, Search, Eye, Edit, Trash2, Copy, Headphones,
...
export default function AdminListeningTestsPage() {
  const tests = [
    { id: "listening-test-001", title: "IELTS Listening Practice Test 01", difficulty: "medium", status: "published", access: "free", attempts: 342, parts: 4, questions: 40, created: "Jan 5, 2024" },
    { id: "listening-test-002", title: "IELTS Listening Practice Test 02", difficulty: "hard", status: "draft", access: "paid", attempts: 0, parts: 4, questions: 40, created: "Jan 12, 2024" },
    { id: "listening-test-003", title: "IELTS Listening Practice Test 03", difficulty: "easy", status: "published", access: "paid", attempts: 178, parts: 4, questions: 40, created: "Dec 20, 2023" },
  ];

  return (
    <DashboardLayout title="Listening Tests" subtitle="Create and manage IELTS Listening mock tests.">
      {/* Actions */}
      <div className="flex items-center justify-between mb-6">
        <div className="relative w-72">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
          <input type="text" placeholder="Search listening tests..."
        ...
        <Link href="/admin/listening-tests/create">
            ...
            <tbody className="divide-y divide-gray-50">
              {tests.map((test) => (
                <tr key={test.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{test.questions} questions</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">{test.parts}</td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-1">
                      <Link href={`/listening/${test.id}`}>
                        <button className="p-1.5 rounded hover:bg-gray-200 text-gray-500" title="Preview"><Eye className="w-4 h-4" /></button>
                      </Link>
                      <Link href={`/admin/listening-tests/${test.id}/edit`}>
